...
/**
 * This class is a REST controller that handles requests regarding enums associated with a module manual.
 */
@Data
@AllArgsConstructor
@RestController
...
public class ModuleManualEnumController {
	...
	private final ModulhandbuchBackendMapper modulhandbuchBackendMapper;
		...
		/**
	 * This method handles GET requests to the `/module-manuals/{id}/requirements` endpoint where id is a variable integer.
	 * It then uses the id to find the mapped data set in the database. If it finds one, it returns a set of strings which represent all the possible values of the enum `admissionRequirement`. If the module manual for the given id could not be found, it throws a {@link ModuleManualNotFoundException} .
	 * 
	...
	/**
	 * This method handles PUT requests to the `/module-manuals/{id}/requirements` endpoint where id is a variable integer.
	 * It then uses the id to find the mapped data set in the database. If it finds one, it replaces the current values of the enum `admissionRequirement` with new values. It then returns a set of strings which represent the updated enum.
	 * 
	 * @param requirements A set with the new values for enum `admissionRequirement` to replace the existing enum with.
	 * @return A set of strings representing the updated enum `admissionRequirement`.
	 */
